...
function UF:Construct_PowerBar(frame, bg, text, textPos)
	...
	power.power_backdrop = CreateFrame('StatusBar', nil, power)
    power.power_backdrop:SetReverseFill(true)
...
function UF:Configure_Power(frame)
	...
	if frame.USE_POWERBAR then
		...
		if (not self.thinBorders and not E.PixelMode) and frame.POWERBAR_HEIGHT < 7 then --A height of 7 means 6px for borders and just 1px for the actual power statusbar
			...
			if db.power.power_backdrop then db.power.power_backdrop.height = 7 end
			heightChanged = true
		elseif (self.thinBorders or E.PixelMode) and frame.POWERBAR_HEIGHT < 3 then --A height of 3 means 2px for borders and just 1px for the actual power statusbar
			...
			if db.power.power_backdrop then db.power.power_backdrop.height = 3 end
			heightChanged = true
		end
		...
		elseif frame.USE_POWERBAR_OFFSET then
			...
			power.power_backdrop:SetFrameLevel(frame.Health:GetFrameLevel() -5) --Health uses 10
...
function UF:PostUpdatePower(unit, cur, min, max)
	...
	self.power_backdrop:SetMinMaxValues(0, max)
  
 	if(disconnected) then
		self.power_backdrop:SetValue(min or cur)
 	else
		self.power_backdrop:SetValue(max-cur)
	end

	local pType, _, altR, altG, altB = UnitPowerType(unit)
	local colors = E.db['unitframe']['colors']['power'].power_backdrop;
	local power_alpha = E.db['unitframe']['colors']['power'].power_alpha;
	local color = ElvUF['colors'].power[tokens[pType]]
	
	self.backdrop.backdropTexture:SetVertexColor(0,0,0,0)
	self.backdrop:SetBackdropColor(0,0,0,0)
	
	if not UF.db.colors.invertpower then
		self.power_backdrop:SetStatusBarColor(colors.r, colors.g, colors.b, colors.a)
		if not self.colorClass then
			self:SetStatusBarColor(color[1], color[2], color[3], power_alpha.a or 1.0)
		end
	else
		self:SetStatusBarColor(colors.r, colors.g, colors.b, colors.a)
		if not self.colorClass then
			self.power_backdrop:SetStatusBarColor(color[1], color[2], color[3], power_alpha.a or 1.0)
		end	
	end
	
	if UF.db.colors.texture_power then	
		self:SetStatusBarTexture(LSM:Fetch("statusbar", UF.db.statusbar))	
	else
        self:SetStatusBarTexture(self:GetStatusBarColor())
	end
	
	if UF.db.colors.backdropTex_power then
		self.power_backdrop:SetStatusBarTexture(LSM:Fetch("statusbar", UF.db.statusbar_backdrop))
	else
